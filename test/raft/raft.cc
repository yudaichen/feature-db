...
void PrintOutput(std::shared_ptr<raft::objfactory<raft::server> > factory,
    ...
    std::ostringstream o;
    osspack(o, "(", id, " ", server->Term(), " ", (int) server->GetState(), ") ",
...
void raft::server::Start() {
    ...
    m_log_vec.clear();
    m_log_vec.push_back(
        Log{(int) m_log_vec.size(), 0, true, "Start"}); // 初始化一条日志
...
void raft::server::ReStart() {
    ...
    // m_votedfor = 0;
    // m_log_vec.clear();
    // m_log_vec.push_back(Log{(int)m_log_vec.size(), 0, true, "Start"}); //
    // 初始化一条日志
    ...
    while (true) {
        ...
        if (m_state == State::Leader && !m_match_index_vec.empty()) {
            ...
            const auto &mid_index =
                    match_vec[(int) match_vec.size() / 2]; // 超过半数提交
...
void raft::server::Election() {
    while (true) {
        ...
        // 发起请求投票
        const VoteArgs &args{
            m_term, (int) m_id, (int) m_log_vec.size() - 1,
...
void raft::server::ReplyVote(const VoteReply &reply) {
    std::unique_lock<std::mutex> _(m_mutex);
    if (m_is_stop ||
        m_state != State::Candidate) // 收到投票返回时可能不是候选人了
        return;
    ...
    if (args.term < m_term) {
        ...
        if (m_commit_index >= args.pre_log_index + (int) args.log_vec.size()) {
            ...
            if ((args.pre_log_index >= (int) m_log_vec.size()))
                ...
                PRINT("not_match ", args.pre_log_index, " >= 0 && ",
                  m_log_vec[args.pre_log_index].term, " != ", args.pre_log_term);

            // 领导记录关于我的提交进度，与我实际进度不一致，则返回失败
            reply.success = false;
        } else {
            PRINT("log_push ", m_commit_index, " ", args.commit_index, " ",
...
void raft::server::ReplyAppendEntries(const AppendEntriesReply &reply) {
    ...
    if (reply.success) {
        ...
        if (reply.log_count > 0)
            PRINT("succ ", reply.id, " ", m_match_index_vec[reply.id], " ",
...
void raft::server::ToLeader() {
    ...
    m_log_vec.push_back(Log{
        (int) m_log_vec.size(), m_term, true,
